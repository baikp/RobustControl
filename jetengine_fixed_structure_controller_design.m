...
Ts = 0.01;
k = ceil(T_delay/Ts);
delay_dtf = tf([-0.5*0.1 1],[0.5*0.1 1],Ts);
%%

%Define the uncertain parameters a,b,c.
a = ureal('a',0.35,'Range',[0.2 0.5]);
b = ureal('b',T_delay,'Range',[0.05 0.15]);
...
G_nominal= tf(1,[0.35 1])*tf([-0.5*T_delay 1],[0.5*T_delay 1]);
...
W1 = makeweight(33,0.2*2*pi,0.5);

W3 = makeweight(0.5,3.0*2*pi,20);
...
F0 = tf(1,[1 a],Ts);    % filter parameterized by a

% Label the block I/Os
Wn = W2_discrete;    Wn.u = 'nw';   Wn.y = 'n';
Wr = W3_discrete;    Wr.u = 'rw';   Wr.y = 'r';
...
% T0 = connect(G,Wn,We,C0,F0,Sum1,Sum2,Sum3,{'r','nw','dw'},{'y','ew'});
T0 = connect(G,Wr,Wd,Wn,C0,F0,Sum1,Sum2,Sum3,Sum4,{'rw','nw','dw'},{'y'});
...
%%
% C0 = C;
% F0 = F;    % filter parameterized by a
% 
% % Label the block I/Os
% 
% Sp = (1 - delay_dtf)*tf(1,[0.35 1],Ts); Sp.u = 'us'; Sp.y = 'ysp';
...
% C0.u = 'e';   C0.y = 'u';
% F0.u = 'r';  F0.y = 'ky';
% G = G_uncertain_discrete; G.u = 'us'; G.y = 'yd';
% % G = G_uncertain; G.u = 'us'; G.y = 'yd';
% 
% % Specify summing junctions
% Sum1 = sumblk('e = r - yf');
...
% % Connect the blocks together
% 
% T1 = connect(G,C0,F0,Sum1,Sum2,Sum3,Sum4,{'r'},{'y'});
% 
% time = 0:0.01:10;
% step_input = ones(size(time)); 
% step_input(1:100) = 0;
% 
% lsim(T1, step_input, time);
% grid on;
figure;
...
% figure;
% step((F*1.0 + C)*G_set_discrete/(1 + G_set_discrete*C)), grid, title('Closed-loop response');
% hold on;
% step(G_nominal);
% hold off;